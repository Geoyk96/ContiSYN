...
from metrics.impute_metrics import impute_metrics
...
class ACTE(torch.nn.Module):

    def __init__(self, func, func_g, input_channels, hidden_channels, output_channels, attention_channel, slope_check,
                 soft, timewise, file, initial=True):
        """
        Arguments:
            func: As cdeint.
            func_g: As cdeint
    ...
    def forward(self, times, coeffs, final_index, slope, z0=None, stream=True, savelabel=False, **kwargs):
        """
        Arguments:
        ...
        if stream:
            ...
            if len(times) - 1 in sorted_final_index:
                sorted_final_index = sorted_final_index[:-1]
            sorted_final_index = sorted_final_index.to(torch.long)
        ...
        if self.soft:
            ...
            if self.slope_check:
                ...
                attention = sigmoid(attention)
                attention = self.binarizer(attention)
        atten_temp = np.array(attention.detach().numpy)
        if savelabel:
            np.save("attention_curve/atten/atten_{}.npy".format(kwargs['data']),atten_temp)
...
class RecoveryODENetwork(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, gru_input_size, x_hidden, delta_t,
                 last_activation='identity', solver='euler'):
        ''' 24 24 6 24 48
        Arguments:
...
class Multi_Layer_ODENetwork(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, gru_input_size, x_hidden, delta_t, num_layer,
                 last_activation='identity', solver='euler'):
        ...
        if num_layer == 1:
            self.model = RecoveryODENetwork(input_size=input_size, hidden_size=hidden_size, output_size=output_size,
                                            gru_input_size=gru_input_size, x_hidden=x_hidden,
                                            last_activation=self.last_activation, delta_t=delta_t, solver=solver)
        elif num_layer == 2:
            ...
            for i in range(num_layer):
                if i == 0:
                    self.model.append(
                        First_ODENetwork(input_size=input_size, hidden_size=hidden_size, output_size=hidden_size,
                                         gru_input_size=gru_input_size, x_hidden=x_hidden, delta_t=delta_t,
                                         solver=solver))
                elif i == num_layer - 1:
                    self.model.append(
                        Last_ODENetwork(input_size=hidden_size, hidden_size=hidden_size, output_size=output_size,
                    ...
                    self.model.append(
                        Mid_ODENetwork(input_size=hidden_size, hidden_size=hidden_size, output_size=hidden_size,
                                       gru_input_size=gru_input_size, x_hidden=x_hidden, delta_t=delta_t,
                                       solver=solver))
...
def train(
    ...
    :type gamma: object
    """
    def _loss_e_t0(x_tilde, x,mask):
        x_tilde=(1-mask)*x_tilde
        x=(1-mask)*x
        return F.mse_loss(x_tilde, x)
    def _loss_e_t1(x_tilde, x):
        return F.mse_loss(x_tilde, x)
    ...
    fname = here / args.save_dir/'{}_pretrained_{}'.format(args.data,args.missing_value)
    os.makedirs(fname, exist_ok=True)
    print(recovery)
    print(discriminator)
    fname = fname / 'embedder_{}.pt'.format(str(args.first_epoch))
    # os.makedirs(fname, exist_ok=True)
    ...
    #    load = True
        ...
        for step in range(1, args.first_epoch + 1):
            ...
            ###########################################
            h = embedder(time, train_coeffs, final_index, slope)
            # h = embedder(x)
            ###########################################
            ...
            loss_e_t0 = _loss_e_t1(x_tilde_no_nan, x_no_nan)
            ...
            if step % 500 == 0:
                path = here / args.save_dir/'{}_pretrained_{}'.format(args.data,args.missing_value)
                torch.save(embedder.state_dict(), path / "embedder_{}.pt".format(str(step)))
                torch.save(recovery.state_dict(), path / "recovery_{}.pt".format(str(step)))
        print("Finish Embedding Network Training")
    print("load embedder,recovery")
    path = here / args.save_dir/'{}_pretrained_{}'.format(args.data,args.missing_value)
    embedder.load_state_dict(torch.load(path / "embedder_{}.pt".format(str(args.first_epoch))))
    recovery.load_state_dict(torch.load(path / "recovery_{}.pt".format(str(args.first_epoch))))
    ...
    for step in range(1, max_steps + 1):
        ...
        if step % 500 == 0:
            ...
            ####################################
            path = args.save_dir/"{}_{}".format(args.data,args.missing_value)
            os.makedirs(path, exist_ok=True)
            torch.save(embedder.state_dict(), path /
                       "embedder{}_lam_{}.pt".format(str(step),args.lam))
            torch.save(recovery.state_dict(), path /
                       "recovery{}_lam_{}.pt".format(str(step),args.lam))
            torch.save(generator.state_dict(), path /
                       "generator{}_lam_{}.pt".format(str(step),args.lam))
            torch.save(discriminator.state_dict(), path /
                       "discriminator{}_lam_{}.pt".format(str(step),args.lam))

            seq_len = x.shape[1]
            input_size = x.shape[2] - 1
            if args.data == 'stock':
                data_path = here / 'datasets/stock_data.csv'
                test_dataset = TimeDataset_irregular(data_path, args.seq_len, args.data, args.missing_value,train=False)
                input_size = 6
            if args.data == 'energy':
                data_path = here / 'datasets/energy_data.csv'
                test_dataset = TimeDataset_irregular(data_path, args.seq_len, 'energy', args.missing_value,train=False)
                # original_dataset = TimeDataset(data_path, args.seq_len)
                input_size = 28
            if args.data == 'ETT':
                data_path = here / 'datasets/ETT_data.csv'
                test_dataset = TimeDataset_irregular(data_path, args.seq_len, 'ETT', args.missing_value,train=False)
                # original_dataset = TimeDataset(data_path, args.seq_len)
                input_size = 7
            dataset_size = test_dataset.size

            with torch.no_grad():
                batch = test_dataset[dataset_size]
                x = batch['data'].to(device)
                train_coeffs = batch['inter']  # .to(device)
                original_x = batch['original_data'].to(device)
                obs = x[:, :, -1]
                x = x[:, :, :-1]
                nan_mask = np.isnan(x.cpu().numpy())
                # 将布尔值数组转换为整数值数组，NaN 对应 0，其余对应 1
                mask = np.where(nan_mask, 0, 1)
            ...
            rmse,mae,rmse2,rmse3=impute_metrics(generated_data1,generated_data2,mask)
            np.save(path/"{}_h_{}.npy".format(args.data,args.hidden_size),np.array(generated_data2))
            metric_results = dict()
            ...
            metric_results['discriminative_std'] = 0


            #metric_results['predictive'] = np.mean(predictive_score)
            #metric_results['predictive_std'] = np.std(predictive_score)
            metric_results['predictive'] = 0
            metric_results['predictive_std'] = 0
            print(metric_results)
            Excel_Operate.add_to_data(path , "stock_{}_数据导出.xlsx".format(args.missing_value), args,step, metric_results['discriminative'], metric_results['discriminative_std'], metric_results['predictive'],
                                      metric_results['predictive_std'],rmse,rmse2,rmse3,mae, sheetname=u"实验数据")
...
def main():
    ...
    parser.add_argument("--data", type=str, default="ETT")
    parser.add_argument("--model1", type=str, default='ACTE')
    ...
    parser.add_argument("--save_dir", type=str, default='energy_test')
    parser.add_argument("--d_layer", type=int, default=1)
    parser.add_argument("--r_layer", type=int, default=2)
    parser.add_argument("--a_layer", type=int, default=4,help="Attention layers")
    ...
    parser.add_argument("--first_epoch", type=int, default=18)
    parser.add_argument("--log_time", type=int, default=1)
    parser.add_argument("--missing_value", type=float, default=0.3)
    parser.add_argument('--slope_check', type=bool, default=False, help="Slope")
    ...
    if args.data == 'energy':
        data_path = here / 'datasets/energy_data.csv'
        dataset = TimeDataset_irregular(data_path, args.seq_len, 'energy', args.missing_value)
        # original_dataset = TimeDataset(data_path, args.seq_len)
        input_size = 28
    if args.data == 'ETT':
        data_path = here / 'datasets/ETT_data.csv'
        dataset = TimeDataset_irregular(data_path, args.seq_len, 'ETT', args.missing_value)
        # original_dataset = TimeDataset(data_path, args.seq_len)
        input_size = 7
    hidden_size = 24
    x_hidden = 48
    #hidden_size = args.hidden_size
    #x_hidden = hidden_size

    attention_size = args.attention_channel
    attention_attention_size = args.attention_attention_channel
    a_layers = args.a_layer
    ...
    initial = True
    if args.model1.upper() == 'ACTE':
        ode_func_g = FinalTanh(input_size, hidden_size, hidden_size, a_layers)
        ode_func_f = FinalTanh_ff6(input_size, attention_size, attention_attention_size, a_layers)
        embedder = ACTE(func=ode_func_f, func_g=ode_func_g, input_channels=input_size,
                         hidden_channels=hidden_size, output_channels=hidden_size, attention_channel=attention_size,
                         slope_check=args.slope_check, soft=args.soft, timewise=args.timewise, file=args.file,
                         initial=initial).to(device)
        recovery = Multi_Layer_ODENetwork(input_size=hidden_size, hidden_size=hidden_size, output_size=input_size,
        ...
        discriminator = Multi_Layer_ODENetwork(input_size=input_size, hidden_size=hidden_size, output_size=1,
                                               ...
                                               last_activation=args.last_activation_d, num_layer=args.d_layer,
                                               delta_t=0.5).to(device)
    else:
        raise ValueError(f"Unsupported model1: {args.model1}")
    ...
    if args.train:
        ...
        here = pathlib.Path(__file__).resolve().parent
        path = here/args.save_dir
        embedder.load_state_dict(torch.load(path/"{}_{}/embedder{}.pt".format(args.data,args.missing_value,args.max_steps)))
        recovery.load_state_dict(torch.load(path / "{}_{}/recovery{}.pt".format(args.data,args.missing_value,args.max_steps)))
        generator.load_state_dict(torch.load(path / "{}_{}/generator{}.pt".format(args.data,args.missing_value,args.max_steps)))
        recovery.load_state_dict(torch.load(path/  "{}_{}/recovery{}.pt".format(args.data,args.missing_value,args.max_steps)))
        ...
        with torch.no_grad():
            ...
            # 将布尔值数组转换为整数值数组，NaN 对应 0，其余对应 1
            mask = np.where(nan_mask, 0, 1)
            np.save("Imputation_curve/data/mask_{}_{}.npy".format(args.data, args.missing_value), mask)
            m = torch.tensor(mask).to(device)
            ...
            ###########################################
            h = embedder(time, train_coeffs, final_index, slope,savelabel=True)
            times = time
            times = times.unsqueeze(0)
            times = times.unsqueeze(2)
            times = times.repeat(obs.shape[0], 1, 1)
        ...
        generated_data_curr = x.cpu().numpy()
        np.save("Imputation_curve/data/orgin_{}_{}.npy".format(args.data,args.missing_value),generated_data_curr)
        generated_data1 = list()
        for i in range(dataset_size):
            temp = generated_data_curr[i, :, :]
            generated_data1.append(temp)
        ...
        generated_data_curr = x_hat.cpu().numpy()
        np.save("Imputation_curve/data/Imputation_{}_{}.npy".format(args.data, args.missing_value), generated_data_curr)
        generated_data2 = list()
        for i in range(dataset_size):
            temp = generated_data_curr[i, :, :]
            generated_data2.append(temp)
        ...
        metric_results['predictive'] = 0
        metric_results['predictive_std'] = 0
        print("{}_{}_{}_{}".format(rmse,rmse2,rmse3,mae))
