...
class TimeDataset_irregular(torch.utils.data.Dataset):
    def __init__(self, data_path, seq_len, data_name, missing_rate = 0.0,train=True):
        # import pdb;pdb.set_trace()
        base_loc = here / 'datasets'
        loc = here / 'datasets'/(data_name+str(missing_rate))
        if os.path.exists(loc):
            ...
            original_sample = tensors['original_data']

            # 将数据转换为numpy数组
            samples = np.array(samples)
            original_sample = np.array(original_sample)
            train_a=tensors['train_a']
            train_b=tensors['train_b']
            train_c=tensors['train_c']
            train_d=tensors['train_d']
            ...
            train_original_sample = original_sample[:split_index]
            test_original_sample = original_sample[split_index:]

            train_coeffs_a = train_a[:split_index,:,:]
            test_coeffs_a = train_a[split_index:,:,:]
            train_coeffs_b = train_b[:split_index,:,:]
            test_coeffs_b = train_b[split_index:,:,:]
            train_coeffs_c = train_c[:split_index,:,:]
            test_coeffs_c = train_c[split_index:,:,:]
            train_coeffs_d=train_d[:split_index,:,:]
            test_coeffs_d=train_d[split_index:,:,:]

            train_coeffs = (train_coeffs_a, train_coeffs_b, train_coeffs_c, train_coeffs_d)
            test_coeffs = (test_coeffs_a, test_coeffs_b, test_coeffs_c, test_coeffs_d)

            # 如果是训练模式
            if train:
                self.samples = train_samples
                self.original_sample = train_original_sample
                self.train_coeffs = train_coeffs
            else:
                self.samples = test_samples
                self.original_sample = test_original_sample
                self.train_coeffs = test_coeffs
            ...
            self.original_sample = []
            norm_data,denom = normalize(data)
            np.save("datasets/denom/denom_{}_{}.npy".format(data_name, missing_rate), denom)

            #sys.exit()

            np.save("attention_curve/atten/norm_data_{}_{}.npy".format(data_name, missing_rate), norm_data)
            ori_seq_data = []
            ...
            np.save("attention_curve/atten/origin_sample_{}_{}.npy".format(data_name, missing_rate), ori_seq_data)
            idx = torch.randperm(len(ori_seq_data))
